...
export class VNAccents{
    constructor (private validationService: ValidationService){}
    
    accents_arr: any = new Array(
    ...
    //     return sa ? str : str[0];
    //   }
    //   remove_vietnamese_accents(str) {
    //     let { accents_arr, no_accents_arr } = this;
    //     return this.str_replace(accents_arr, no_accents_arr, str);
    //   }

      removeAccents(string) {
        let { accents_arr, no_accents_arr } = this;
        if(!this.validationService.checkNotEmpty(string)) return '';
        return string
          .split("")
          .map((letter, index) => {
